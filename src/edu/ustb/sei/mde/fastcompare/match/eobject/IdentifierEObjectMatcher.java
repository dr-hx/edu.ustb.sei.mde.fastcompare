...
public class IdentifierEObjectMatcher implements IEObjectMatcher {
    ...
    public void createMatches(Comparison comparison, Collection<EObject> leftRoots,
        ...
        final TreesIterator leftEObjects = new TreesIterator(leftRoots);
        final TreesIterator rightEObjects = new TreesIterator(leftRoots);
        final TreesIterator originEObjects = new TreesIterator(leftRoots);

        createMatches(comparison, leftEObjects, rightEObjects, originEObjects);
    }
	 ...
	 */
	private void reportDuplicateID(ObjectIndex.Side side, EObject eObject) {
		...
		if (uriString != null) {
			message = "IdentifierEObjectMatcher.duplicateIdWithResource"+duplicateID+sideName+uriString;
		} else {
			message = "IdentifierEObjectMatcher.duplicateId"+duplicateID+sideName+uriString;
		}
			...
			while (leftEObjects.hasNext()) {
				...
				if (identifier != null) {
					...
					if (isAlreadyContained) {
						reportDuplicateID(ObjectIndex.Side.LEFT, left);
					}
				...
				if (identifier != null) {
					...
					if (match != null) {
						if (match.getRight() != null) {
							reportDuplicateID(ObjectIndex.Side.RIGHT, right);
						}
				...
				if (identifier != null) {
					...
					if (match != null) {
						if (match.getOrigin() != null) {
							reportDuplicateID(ObjectIndex.Side.ORIGIN, origin);
						}
